---
date: 2026-02-13
description: Apprenez à convertir des coordonnées en DMS avec Aspose.GIS pour .NET.
  Ce guide C# étape par étape montre comment convertir latitude/longitude, degrés
  décimaux en DMS et bien plus encore.
linktitle: Convert Coordinates
second_title: Aspose.GIS .NET API
title: Comment convertir des coordonnées en DMS avec Aspose.GIS pour .NET
...
# Comment convertir des coordonnées en DMS avec Aspose.GIS

## Introduction
Dans ce tutoriel, vous apprendrez **comment convertir des coordonnées** en DMS (degrés, minutes, secondes) à l’aide de la puissante bibliothèque Aspose.GIS pour .NET. Que vous ayez besoin de **c# convert lat long**, de générer des chaînes de localisation lisibles par l’homme, ou simplement d’explorer différents formats de coordonnées, ce guide vous accompagne pas à pas avec des explications claires et du code C# prêt à l’emploi.
...
## Comment convertir des coordonnées en DMS
Avant de plonger dans le code, clarifions pourquoi le DMS reste utile. Les cartographes, pilotes et de nombreux fournisseurs de données SIG s’appuient sur le DMS car il est convivial et correspond aux cartes traditionnelles. Aspose.GIS supprime le besoin de calculs manuels, vous permettant de vous concentrer sur la logique de votre application.
...
2. **Aspose.GIS installé** – Téléchargez le dernier package depuis le [site Web Aspose.GIS](https://releases.aspose.com/gis/net/).  
...
Même si le but final est le DMS, nous commençons par afficher la représentation décimale d’origine. Cela montre également le chemin **decimal degrees to dms** que vous suivrez ensuite.
...
- **Lettres d’hémisphère incorrectes** – Assurez‑vous de fournir des valeurs positives pour le nord/est et négatives pour le sud/ouest ; l’API ajoute automatiquement le suffixe correct.  
...
## Conclusion
En suivant ces étapes, vous savez maintenant **comment convertir des coordonnées en DMS** et d’autres formats SIG courants à l’aide d’Aspose.GIS pour .NET. Cette capacité vous permet d’intégrer facilement des chaînes de localisation lisibles par l’homme dans des applications cartographiques, des rapports ou tout flux de travail de données spatiales. N’hésitez pas à expérimenter avec différentes valeurs de latitude/longitude et à explorer les autres formats offerts par la classe `GeoConvert`.
