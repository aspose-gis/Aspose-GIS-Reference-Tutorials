...
# 空間參考 wgs84 – 使用 Aspose.GIS 為 GDB 線上圖譜新增圖層
...
將所需的 `using` 語句加入您的 C# 專案中，以便存取 Aspose.GIS 類別：
...
首先，複製包含原始 GDB 資料集的資料夾。保留副本可在您進行實驗時保護來源資料。
...
開啟新複製的資料集，並確認它可以建立新圖層。 `CanCreateLayers` 屬性應傳回 **True**。
...
## 步驟 3：建立並填滿空間參考為 wgs84 的新圖層

現在，我們建立一個名為 **data** 的圖層，並將其空間參考明確設定為 **Wgs84**。我們也新增了一個名為 **Name** 的簡單屬性，並插入一個點要素。
...
最後，打開剛剛建立的圖層並驗證其內容。控制台將顯示該圖層包含一個要素，並且 **Name** 屬性與我們設定的值相符。
...
### 問：Aspose.GIS for .NET 支援哪些空間參考系統？
...
### 問：在哪裡可以找到 Aspose.GIS for .NET 的詳細文件？
